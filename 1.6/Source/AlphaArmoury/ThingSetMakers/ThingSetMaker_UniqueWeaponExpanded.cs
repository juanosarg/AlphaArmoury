...
{
    public class ThingSetMaker_UniqueWeaponExpanded : ThingSetMaker
    {
        ...
        protected override void Generate(ThingSetMakerParams parms, List<Thing> outThings)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                do
                {
                    thing = ThingMaker.MakeThing(DefDatabase<ThingDef>.AllDefs.Where((ThingDef x) => x.HasComp<CompUniqueWeapon>() && x.techLevel<=parms.techLevel).RandomElement());
                }
